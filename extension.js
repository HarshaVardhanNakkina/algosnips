...
function activate(context) {
	...
	vscode.commands.registerCommand('algosnips.search', async () => {
		...
		// once the algorithm is chosen, let the user choose a language
		let chosenLanguage = await showLanguagePicker(chosenAlgorithm).catch(err => {
			vscode.window.showErrorMessage(err.message)
			return
		})
		if (!chosenLanguage) {
		...
		let fetchedGist = await fetchGist(chosenAlgorithm, chosenLanguage).catch(err => {
			vscode.window.showErrorMessage(err.message)
			return
		})
